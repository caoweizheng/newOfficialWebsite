 ...
 * @Last Modified time: 2018-07-15 15:11:43
...
export default class Payconfirm extends React.Component {
  ...
  render() {
    ...
    switch (type) {
      case 'amount':
        userAmount =
          dataType === 1 ? parseFloat(nidosportAmount) : parseFloat(btAmount);
        payWay = dataType === 1 ? '灵动余额支付' : '本汀余额支付';
        ext = '元';
        btnText = '前往充值';
        btnUrl =
          dataType === 1 ? 'https://www.nidosport.com/pay/do' : '/pay/do';
    ...
    return (
      <Modal popup visible={show} onClose={onClose} animationType="slide-up">
        <List
        ...
        >
          <List.Item className="t-title" extra={payWay}>
            付款方式
          </List.Item>
          <List.Item
